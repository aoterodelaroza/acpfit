...
100 format (A,"(",A,"): ",A)
  ...
  subroutine help_me()
    ...
    write (uout,'("")')
    write (uout,'("  FILE ADD addfile.s [coef.r]")')
    write (uout,'("    Add the contents of a file as an additonal energy contribution. The energies in")')
    write (uout,'("    this file will be added to the empty, multiplied by coef.r.")')
    write (uout,'("    If coef.r is not given, then the coefficient is equal to zero.")')
    write (uout,'("    The contents of the addfile.s are passed to the octave dump for the lasso fit, and")')
    write (uout,'("    they are also used during ACP evaluation. In ACP evaluation, the coef.r indicated")')
    write (uout,'("    in this line may be overwritten by the contents of the ACP header.")')
    write (uout,'("    This is useful for additional energies,")')
    write (uout,'("    such as a dispersion energy, where the multiplicative coefficient in front can")')
    write (uout,'("    be changed. (Default: none)")')
    write (uout,'("")')
    ...
    write (uout,'("")')
    write (uout,'("  SET label.s ini.i num.i step.i")')
    write (uout,'("    Define a new subset of the fitting set for separate evaluation. The set starts")')
    write (uout,'("    at system number ini.i and has num.i elements, with step step.i. The step argument")')
    write (uout,'("    is optional (step.i = 1 by default).")')
    write (uout,'("")')
